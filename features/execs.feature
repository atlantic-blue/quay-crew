Feature: A session's history can be read back
  ...
  An exec is written to the store when it starts, so an operator can see what a session was asked
  ...
  Scenario: A conversation reads back in the order it happened
    ...
    Then the session has 2 execs
    And the first exec says "hello" and the second says "and again"

  # An exec used to be written only when it ended. So for the minutes or the hours it ran there was
  ...
  #
  # The exec is held open rather than timed, because what is specified here is what is true *while*
  # one runs, and a scenario that waits a duration for that passes by accident.
  Scenario: An exec is in the history while it is still running
    Given the model takes longer over an exec than anybody will wait
    And an exec dispatched with the caller waiting for it
  ...
  Scenario: An exec nobody is waiting for is visible while it runs too
    Given the model takes longer over an exec than anybody will wait
  ...
  # One exec, not two. The landing closes the record the start opened, so the operator reads the
  # prompt once with the answer under it.
  Scenario: What an exec came to is written into the exec that started
    Given the model takes longer over an exec than anybody will wait
    And an exec dispatched with the caller waiting for it
    And an exec is under way
    When the model finishes the exec
    And the operator's dispatch comes back
    Then the system's one session has 1 exec
