Feature: The first run is guided
  ...
  A system that opens empty showed an empty console, and nothing suggested what to do next. The wizard
  behind n makes one thing at a time, which is right for a system in use and wrong for a first run:
  ...
  stages in the order the system needs them: a workspace, a project in it, the model token, context
  for the project, a skill from what the system holds, and a first message. An empty answer skips a
  ...
  Scenario: An empty system opens into the guided setup
    When the operator opens the console on the empty system
    Then the guided setup is asking for a workspace name

  Scenario: A system with a workspace is not offered the setup
    Given a workspace named "acme"
    When the operator opens the console on the empty system
    Then the console is asking nothing

  Scenario: The guided setup builds a working system in one pass
    ...
    And the operator answers the guided setup with:
      ...
      | notes                  |
      | hello                  |
    Then the system has 1 workspace
  ...
  # A system with no skills has nothing to offer at the skill stage, so the stage passes itself over
  ...
  Scenario: A system holding no skills is not asked about skills
    When the operator opens the console on the empty system
    And the operator answers the guided setup with:
      | acme          |
      | house-bills   |
  ...
  Scenario: Skipping the workspace closes the setup and makes nothing
    ...
    And the operator skips a stage of the guided setup
    Then the console is asking nothing
    And the system has 0 workspaces
