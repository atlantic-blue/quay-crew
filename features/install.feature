Feature: A first run is one command
  ...
  crew rather than a missing step.
  ...
  replaces the services under a crew that is already working without saying what that costs and
  ...
  These scenarios run the real make against a crew directory of their own. Docker is answered by a
  ...
  Scenario: One command leaves a running crew and a quay that runs
    Given a machine with no crew on it
    When the operator runs "make install"
    Then the crew has a configuration file
    And the crew has a data directory
  ...
  Scenario: A crew that is already working is not replaced without a word
    Given a machine with a crew already running
    When the operator runs "make install"
    Then it refuses
    And the stack was never brought up
    And it says the crew is already up
    And it offers "make rebuild"
    And it offers "make install YES=1"

  Scenario: Typing the crew's name back replaces it
    Given a machine with a crew already running
  ...
  Scenario: Rebuilding one part leaves the running crew alone
    Given a machine with a crew already running
  ...
  Scenario: Asking for the configuration alone starts nothing
    ...
    Then it succeeds
    And the crew has a configuration file
    And the stack was never brought up
