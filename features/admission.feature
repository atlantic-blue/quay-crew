Feature: The system admits work the machine can host, and holds the rest
  ...
  So the system does the arithmetic kubernetes does. A sandbox declares what it needs. The system reads
  ...
  it manages. The system's own control plane, database and event log are containers inside the same
  runtime the work fills, so the system holds capacity back for itself or it goes down with its own
  workload.
  ...
  # wide. The system counts what it has promised as well as what it has built.
  Scenario: A burst of jobs is admitted only as far as the machine goes
    ...
    When the controller ticks
    Then the system was asked to run 3 tasks
  ...
  # Rule five of the headroom issue, applied to admission: a system that cannot read its machine has no
  # arithmetic to do. Refusing everything would stop dead every system whose sessions do not run on a
  # container runtime at all.
  Scenario: A system that cannot read its machine still runs the work
