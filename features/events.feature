Feature: Every task is exported to the event log, when there is one

  The store holds the truth: a task is written to it in the same breath as the task itself, and
  ...
  Scenario: A task that worked is published to the workspace's stream
    When the operator dispatches "hello" to the project
    Then 1 task is on the log for "acme"
    And the published task says "hello" was asked and "you said: hello" came back
    And the published task is keyed by its session

  Scenario: The record says where the session sits, without anyone having to look it up
    When the operator dispatches "hello" to the project
    Then the published task carries the workspace, the project and the session

  Scenario: A task that failed is published too, with what went wrong
    Given the next task will fail
    When the operator dispatches "hello" to the project
    Then 1 task is on the log for "acme"
    And the published task failed and says why

  Scenario: Every task of a conversation is on the log, in order
    When the operator dispatches "hello" to the project
    And the operator dispatches "and again" to the same session
    Then 2 tasks are on the log for "acme"
    And the tasks on the log are in the order they were asked
  ...
  Scenario: Two workspaces do not share a stream
    ...
    Then 1 task is on the log for "acme"
    And 1 task is on the log for "beta"
