...
// The scenarios about why an exec failed run the real model adapter, because the explanation is built
...
// refusalStream is the shape the model reports a refused exec in: on standard output, on the same
// result event a reply arrives on. Taken from a real exec against a rejected token.
...
func initializeFailureSteps(sc *godog.ScenarioContext) {
	// useTheRealRunner swaps the recording double for the real adapter and restarts, so the exec goes
	...
	sc.Step(`^the model refuses the exec saying "([^"]*)"$`, func(ctx context.Context, reason string) error {
		w := worldFrom(ctx)
	...
	sc.Step(`^the model refuses the exec quoting the token back$`, func(ctx context.Context) error {
		w := worldFrom(ctx)
	...
	sc.Step(`^the refusal says "([^"]*)"$`, func(ctx context.Context, want string) error {
		w := worldFrom(ctx)
		if w.lastErr == nil {
			return fmt.Errorf("the exec reported success, so there is no refusal to read")
		}
