...
// The scenarios about why a turn failed run the real model adapter, because the explanation is built
...
// refusalStream is the shape the model reports a refused turn in: on standard output, on the same
// result event a reply arrives on. Taken from a real turn against a rejected token.
...
func initializeFailureSteps(sc *godog.ScenarioContext) {
	// useTheRealRunner swaps the recording double for the real adapter and restarts, so the turn goes
	...
	sc.Step(`^the model refuses the turn saying "([^"]*)"$`, func(ctx context.Context, reason string) error {
		w := worldFrom(ctx)
	...
	sc.Step(`^the model refuses the turn quoting the token back$`, func(ctx context.Context) error {
		w := worldFrom(ctx)
	...
	sc.Step(`^the refusal says "([^"]*)"$`, func(ctx context.Context, want string) error {
		w := worldFrom(ctx)
		if w.lastErr == nil {
			return fmt.Errorf("the turn reported success, so there is no refusal to read")
		}
