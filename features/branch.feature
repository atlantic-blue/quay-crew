Feature: One branch carries a requirement from its failing tests to the build that turns them green
  ...
  The worker that writes a requirement's tests cuts the branch the system named, pushes it and opens
  the pull request from it, and that pull request is the one the work lands in: it stays open and
  red, carrying the failing tests for that requirement and nothing else. The worker that builds the
  same requirement fetches the branch, checks it out and turns those tests green on it, in the same
  pull request. The build stage opens none of its own.
  ...
  Scenario: Each requirement's tests are opened in a pull request of their own, left red
