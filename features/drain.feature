Feature: The system is put down before something takes its containers
  ...
  says stopped and the sandbox is closed rather than ripped out, and an exec still working refuses the
  ...
  # The whole reason this exists. An exec in flight is somebody's work, and the upgrade that would have
  # taken it says nothing at all today.
  Scenario: An exec still working refuses the drain
  ...
  # operator waits for the exec, drains again, and finds every other conversation already down.
