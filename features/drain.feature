Feature: The crew is put down before something takes its containers
  ...
  Draining is the same teardown asked for through the crew. Each session is stopped first, so the row
  ...
  Scenario: Draining puts a live session down and closes its sandbox
    ...
    When the operator drains the crew
  ...
  Scenario: Draining a crew with nothing live puts nothing down
    When the operator drains the crew
  ...
  Scenario: A session already stopped is not put down twice
    ...
    And the operator drains the crew
    Then the drain says it put down 0 sessions
  ...
  # A drain that stopped half the crew and then refused would be worse than one that did nothing: the
  ...
  Scenario: A refused drain leaves every session alone
    ...
    And a task is under way
    When the operator drains the crew
    Then no sandbox the crew made is closed
  ...
  Scenario: Draining anyway puts down the session that was working
    ...
    And a task dispatched without waiting for it
    And a task is under way
    When the operator drains the crew anyway
