...
// initializeWizardSteps registers the steps for making something from the console. They drive the
// console's own reducer against the real control plane, which is the only way to say that the wizard
// came back to a list carrying what it had just made.
func initializeWizardSteps(sc *godog.ScenarioContext) {
	sc.Step(`^the operator makes something from the wizard, answering:$`, func(ctx context.Context, answers *godog.Table) error {
		c := consoleFrom(ctx)
	...
	// The list underneath was refreshed the whole time. This is what nobody could see.
...
// openWizard builds the real console over the live control plane and presses the key that makes
