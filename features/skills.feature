Feature: A session is given the skills the system has
  ...
  # A system with no skills is every system before this, and the memory file must not grow a heading for
  ...
  # number behind that is measured: this system's context reached 51,727 bytes at the workspace.
  Scenario: The memory file names a skill and where to read it
    Given the system has a skill "git" that says "Branch first. Stage named files."
    When the operator dispatches "hello" to the project
  ...
  Scenario: The index is never taken into the system's own context
    ...
    Then no context the system holds mentions the git skill
  ...
  Scenario: Setting the secret is enough for the next sandbox to hold the skill
    ...
    Then the newest sandbox mounts the system's github skill read only

  Scenario: A skill needing a binary the image does not carry is refused, and names the image
    Given the system has a skill "github" needing the binary "gh"
  ...
  Scenario: A skill naming the system's own configuration is refused at import
    When the operator imports a skill whose manifest names the secret "QC_TOKEN"
    Then the system refuses it saying "the system's own"
    And the system holds no imported skills
  ...
  # The system's skills directory is one way in and reaches every session. The other is importing a skill
  # into the store and attaching it to a workspace, which is where a credential belongs: a token for one
  # capability should not be handed to every session the system has.
  #
  # The files travel rather than the path, because the control plane runs in a container where a
  # directory on the operator's machine means nothing, and a system on a pod has no host directory to go
  ...
  Scenario: A skill is imported and the system says what it can do
    When the operator imports the "github" skill
    Then the system holds the "github" skill
    And the listing says the skill needs "git"
    And the listing names the secret "GH_TOKEN" and what it is for

  # A skill given to the system is held by every workspace, including the ones made after it, which is
  ...
  Scenario: A skill the system holds reaches a workspace that attached nothing
    Given the operator imported the "github" skill
    And the workspace has the secret "GH_TOKEN" set to "ghp-1234"
    When the operator attaches the "github" skill to the system
  ...
  Scenario: The listing says a skill came from the system
    ...
    And the operator attached the "github" skill to the system
    When the operator lists the workspace's skills
    Then the listing says the "github" skill is held by the system

  # Two separate statements, and the wider one does not undo the narrower one.
  Scenario: Taking a skill off the system leaves a workspace's own attachment alone
    Given the operator imported the "github" skill
    And the operator attached the "github" skill to the system
    And the operator attached the "github" skill to the workspace
    When the operator detaches the "github" skill from the system
  ...
  # passes just as happily against a system attach that never did anything.
  Scenario: Taking a skill off the system takes it off a workspace that only had it that way
    Given the operator imported the "github" skill
    And the operator attached the "github" skill to the system
  ...
  # A system that starts with nothing makes every operator do the same setup before it can do anything:
  # import each skill, then attach each one to each workspace. The files are in the image already, so
  # a fresh system is simply given them.
  #
  ...
  Scenario: A fresh system is given the skills this build ships with
    When the system starts, seeded from the skills this build ships with
    Then the workspace holds the "git" skill
    And the listing says the "git" skill is held by the system

  Scenario: A skill a fresh system was only offered is in the catalogue and not in front of anybody
    When the system starts, seeded from the skills this build ships with
  ...
  Scenario: Starting again leaves what the operator decided alone
    ...
    And the operator detaches the "github" skill from the system
    When the system starts, seeded from the skills this build ships with
    Then the workspace does not hold the "github" skill

  Scenario: A system that already holds a skill is not seeded over
    Given the operator imported the "github" skill
    When the system starts, seeded from the skills this build ships with
    Then the workspace holds no skills
  ...
  Scenario: Importing a different skill at the same version is refused
    ...
    Then the system refuses it saying "already imported and is a different skill"
  ...
  Scenario: A brief edited on the host does not survive
    ...
    And the operator dispatches "and again" to the same session
    Then the "github" brief reads what the system holds

  # A name can be held by both: the system's directory and a workspace that imported one. Two mounts on one
  ...
  Scenario: A workspace's own skill wins over the system's of the same name
    Given the system has a skill "github" that says "the system's own version"
    And the operator imported the "github" skill
    And the workspace has the secret "GH_TOKEN" set to "ghp-1234"
    When the operator attaches the "github" skill to the workspace
    And the operator dispatches "hello" to the project
  ...
  # A private key is the most sensitive thing this system would carry, so a workspace that has not
