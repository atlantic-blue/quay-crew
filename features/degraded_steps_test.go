...
// Steps for what the command line tool says about a crew that is reporting itself not serving.
//
// The tool runs as its own process against a crew on a real address, so a scenario reads the two
// streams a caller reads. That is the whole finding: the crew knew, and nothing the operator typed
// said so.
...
const notServing = "this crew is not serving"

func initializeDegradedSteps(sc *godog.ScenarioContext) {
	sc.Step(`^standard error says this crew is not serving and names "([^"]*)"$`,
	...
	sc.Step(`^standard output carries the answer and nothing about the crew being down$`,
	...
	sc.Step(`^standard error says nothing about the crew being down$`, func(ctx context.Context) error {
		if got := toolFrom(ctx).stderr; strings.Contains(got, notServing) {
			return fmt.Errorf("standard error calls a crew nothing has probed not serving: %q", got)
		}
		return nil
	})
}
