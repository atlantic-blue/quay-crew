Feature: The crew says how much room the machine has left
  ...
  drawing a healthy crew. Every number that mattered had to be read from outside quay with
  `docker stats`.

  So the crew reads the daemon it already talks to, on its own timer, and reports four things: what
  ...
  these numbers, so a number the crew guessed is a session stopped for nothing.
  ...
  Scenario: The operator reads what the machine holds against the limit that binds
    ...
    Then the crew says it holds "3628 MiB" of "7837 MiB"
    And the crew says the machine is "room"
  ...
  Scenario: A machine near the daemon's own cap reads full
    ...
    And the crew reads the machine
    When the operator asks how much room there is
    Then the crew says the machine is "full"

  # The kill came from the machine underneath while the daemon was at less than half its cap, so the
  # two are reported apart. A crew that reported only the daemon would have said there was room.
  Scenario: The machine underneath is reported apart from the daemon
    ...
    And the crew reads the machine
    When the operator asks how much room there is
    Then the crew says the machine is "room"
    And the crew says the swap is "16402 MiB" of "17408 MiB"

  # Rule five of the issue: no number the crew cannot measure. Unknown is a valid answer and a guess
  # is not, and a crew that could not read the machine must never say there is room.
  Scenario: A crew that cannot read the machine says unknown rather than a number
    Given the crew cannot read the machine
    And the crew reads the machine
    When the operator asks how much room there is
    Then every figure reads unknown
    And the crew says the machine is "unknown"
    And the crew says why it knows nothing
    And the crew still answers everything else
  ...
  # header reads the crew's last sample and the crew reads the machine on a timer of its own.
  Scenario: Asking the crew never reads the machine
    Given the machine holds 3628 megabytes of a 7837 megabyte limit
    And the crew reads the machine
    When the operator asks how much room there is
