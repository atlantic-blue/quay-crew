...
//
// A file the image writes to /home/agent/.claude is a file no session ever reads: the system mounts the
...
// refused here instead. What the runtime is told is rendered by the system, in internal/hook.
func TestTheImageWritesNoSettingsWhereTheSystemMountsOverIt(t *testing.T) {
	for _, line := range strings.Split(theSandboxImage(t), "\n") {
		...
		t.Errorf("the image writes settings the mount hides, so no session reads them:\n%s\n\n"+
			"say it in internal/hook.Settings instead, which the system renders and mounts read only", naked)
	}
}
