...
func runRole(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: krewe role <import|list|show|attach|detach>")
	}
	switch args[0] {
	case "import":
...
func runRoleImport(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe role import <directory>")
	}
	...
	fmt.Fprintf(out, "attach it with: krewe role attach %s\n", imported.GetName())
	return nil
}
...
func runRoleList(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if len(args) > 1 {
		return fmt.Errorf("usage: krewe role list [<workspace>]")
	}
	...
	read := heldBy("roles", where, "krewe role list on its own reads what the system holds")
...
func runRoleAttach(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	// "system" where a workspace goes, the same word krewe skill attach and krewe context set take, and
...
func roleAndAddress(args []string, verb string) (name, typed string, err error) {
	...
	default:
		return "", "", fmt.Errorf("usage: krewe role %s [<workspace>] <name>", verb)
	}
}
