...
// `krewe stop <session> [<reason>]` at the command line.
//
...
func TestStopWithNoSessionSaysHowToUseIt(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("krewe stop with no session was accepted: %q", out.String())
	}
	for _, want := range []string{"krewe stop <session>", "<reason>"} {
...
func TestStopTakesAtMostASessionAndAReason(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("krewe stop took four words and acted on them: %q", out.String())
	}
}
...
func TestStopRefusesASessionTheSystemDoesNotHave(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("krewe stop on a session that does not exist was accepted: %q", out.String())
	}
}
...
func TestStopIsInTheUsage(t *testing.T) {
	...
	if !strings.Contains(said, "stop <session>") {
		t.Fatalf("the usage does not carry krewe stop")
	}
}
