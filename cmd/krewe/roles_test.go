...
// The word system where a workspace goes, which is what krewe skill attach and krewe context set already
...
func TestTheRoleCommandNamesWhatItTakes(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		if !strings.Contains(err.Error(), "usage: krewe role") {
			t.Errorf("krewe %s was refused without saying what to type: %v", strings.Join(args, " "), err)
		}
	}
}
