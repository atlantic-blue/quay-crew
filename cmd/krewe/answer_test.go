...
func TestEveryIdentifierOfASessionReachesItsAnswer(t *testing.T) {
	...
	for _, named := range []string{
		...
		printed, err := runKrewe(t, client, "answer", named)
		if err != nil {
			t.Errorf("krewe answer %s: %v", named, err)
			continue
		}
		if printed != "the 14th\n" {
			t.Errorf("krewe answer %s printed %q, want the reply", named, printed)
		}
	}
}
...
func TestEveryAnswerIsPrintedOldestFirst(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("krewe answer --all: %v", err)
	}
	if printed != "answer 1\nanswer 2\n" {
...
func TestAnswerWithoutASessionSaysHowItIsCalled(t *testing.T) {
	...
	printed, err := runKrewe(t, client, "answer")
	if err == nil {
	...
	if !strings.Contains(err.Error(), "krewe answer <session>") {
...
func TestAFlagAnswerDoesNotTakeIsRefused(t *testing.T) {
	...
	if _, err := runKrewe(t, client, "answer", session.GetId(), "--everything"); err == nil {
