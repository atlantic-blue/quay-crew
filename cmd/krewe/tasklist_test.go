...
func TestAnArchivedSessionsHistoryIsStillReadable(t *testing.T) {
	...
	mustRun(t, client, "task", "read the package file")
	...
	read, err := runKrewe(t, client, "task", "list", session.GetId()[:8])
	if err != nil {
		t.Fatalf("krewe task list on an archived session: %v", err)
	}
...
func TestAnArchivedSessionAnswersToItsHandle(t *testing.T) {
	...
	mustRun(t, client, "workspace", "create", "me")
	mustRun(t, client, "project", "create", "house-bills")
	mustRun(t, client, "task", "remember this")
	...
	read, err := runKrewe(t, client, "task", "list", session.GetHandle()[:8])
	if err != nil {
		t.Fatalf("krewe task list by handle on an archived session: %v", err)
	}
...
func TestASessionNobodyHasIsStillRefused(t *testing.T) {
	...
	_, err := runKrewe(t, client, "task", "list", "nosuchsession")
	if err == nil {
