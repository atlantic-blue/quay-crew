...
func TestASessionScopedCommandTakesTheIdTheListingPrints(t *testing.T) {
	...
	if !strings.Contains(said, "runs in") {
		t.Fatalf("krewe mode %s said %q", id, said)
	}
}
...
func TestASessionScopedCommandTakesTheHandleTheListingPrints(t *testing.T) {
	...
	if !strings.Contains(said, "runs in") {
		t.Fatalf("krewe mode %s said %q", handle, said)
	}
}
...
func TestASessionScopedCommandTakesAnAddress(t *testing.T) {
	...
	if !strings.Contains(said, "runs in") {
		t.Fatalf("krewe mode %s said %q", address, said)
	}
}
...
//
// krewe turns is in fingers, in scripts and in notes. A command that simply stops existing reads as
...
func TestTheOldThreadsCommandIsRefusedAndNamesTheNewOne(t *testing.T) {
	...
	for _, typed := range []string{"threads", "thread"} {
		...
		if !strings.Contains(err.Error(), "krewe sessions") {
			t.Fatalf("the refusal for krewe %s does not say what to type instead: %v", typed, err)
		}
	}
}
...
func TestAttachTakesEveryIdentifierTheListingPrints(t *testing.T) {
	...
	for _, typed := range []string{
		...
		if err != nil {
			t.Fatalf("krewe attach %s was refused: %v", typed, err)
		}
		if reached != session.GetId() {
			t.Fatalf("krewe attach %s reached %s, want %s", typed, reached, session.GetId())
		}
	}
}
