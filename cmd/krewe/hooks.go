...
func runHook(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: krewe hook <import|list|attach|detach>")
	}
	switch args[0] {
	case "import":
...
func runHookImport(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe hook import <directory>")
	}
	...
	fmt.Fprintf(out, "attach it with: krewe hook attach system %s\n", imported.GetName())
	return nil
}
...
func runHookList(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if len(args) > 1 {
		return fmt.Errorf("usage: krewe hook list [<workspace, or workspace/project/session>]")
	}
	...
	read := heldBy("hooks", where, "krewe hook list on its own reads what the system holds")
	...
	if len(resp.GetHooks()) == 0 {
		...
		fmt.Fprintln(out, "import one with: krewe hook import <directory>")
		return nil
	}
...
func runHookAttach(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	// "system" where a workspace goes, the same word krewe context set and krewe skill attach take. It is
...
func hookAndAddress(args []string, verb string) (name, typed string, err error) {
	...
	default:
		return "", "", fmt.Errorf("usage: krewe hook %s [<workspace>] <name>", verb)
	}
}
