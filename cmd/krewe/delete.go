...
func runWorkspaceDelete(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe workspace delete <workspace>")
	}
	...
	if here, err := currentPath(); err == nil && here.Workspace == name {
		...
		fmt.Fprintln(out, "you were standing there, so you are now nowhere: krewe use <workspace>/<project>")
	}
	return nil
}
...
func runProjectDelete(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe project delete [<workspace>/]<project>")
	}
	...
	if located.ProjectID == "" {
		return fmt.Errorf("%q names a workspace, not a project: delete a workspace with krewe workspace delete", args[0])
	}
