...
func mustRun(t *testing.T, client quaycrewv1.ControlPlaneServiceClient, args ...string) string {
	...
	if err := run(context.Background(), client, args, &out, ""); err != nil {
		t.Fatalf("krewe %s: %v", strings.Join(args, " "), err)
	}
	return out.String()
}
...
func TestCreateMovesYouAndTalkingFollows(t *testing.T) {
	...
	if where := mustRun(t, client, "use"); strings.TrimSpace(where) != "me/house-bills" {
		t.Fatalf("krewe use says %q, want me/house-bills", where)
	}

	replied := mustRun(t, client, "exec", "hello", "there")
...
func TestAddressOnTheCommandLineDoesNotMoveYou(t *testing.T) {
	...
	if replied := mustRun(t, client, "exec", "me/gardening", "order the bulbs"); !strings.Contains(replied, "ok") {
...
func TestUseASessionContinuesThatConversation(t *testing.T) {
	...
	mustRun(t, client, "workspace", "create", "me")
	mustRun(t, client, "project", "create", "house-bills")
	first := mustRun(t, client, "exec", flagDispatch, "hello")
	...
	second := mustRun(t, client, "exec", flagDispatch, "and again")
	if got := sessionFrom(t, second); got != session {
		t.Fatalf("the second exec ran in session %s, want the one the context named, %s", got, session)
	}
}
...
func TestDispatchFromAWorkspaceSaysWhichProjectsItHolds(t *testing.T) {
	...
	err := run(context.Background(), client, []string{"exec", flagDispatch, "hello"}, io.Discard, "")
	if err == nil {
...
func TestDispatchWithNowhereToGoExplainsItself(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "krewe use") {
...
func TestFeaturesNeedsNoControlPlane(t *testing.T) {
	...
	if err := run(context.Background(), nil, []string{"features"}, &out, ""); err != nil {
		t.Fatalf("krewe features with no client: %v", err)
	}
	...
	if err := run(context.Background(), nil, []string{"features", "address"}, &out, ""); err != nil {
		t.Fatalf("krewe features address: %v", err)
	}
	...
	if err := run(context.Background(), nil, []string{"features", "quantum"}, &out, ""); err != nil {
		t.Fatalf("krewe features quantum: %v", err)
	}
...
func TestTheOldFlagsAreRefusedRatherThanSwallowed(t *testing.T) {
	...
	for _, invocation := range [][]string{
		{"exec", flagDispatch, "--project", "default", "remember the number"},
		{"exec", flagDispatch, "--project=default", "remember the number"},
		{"sessions", "--workspace", "demo"},
		{"exec", flagDispatch, "--session", "abc123", "hello"},
		{"secret", "set", "--workspace", "demo", "KEY", "value"},
	} {
		err := run(context.Background(), client, invocation, io.Discard, "")
		if err == nil {
			t.Fatalf("krewe %s was accepted, want it refused", strings.Join(invocation, " "))
		}
		if !strings.Contains(err.Error(), "is gone") || !strings.Contains(err.Error(), "krewe use") {
			t.Fatalf("krewe %s said %q, want it to name the flag and how to say it now",
...
func TestAnyOtherFlagIsRefusedToo(t *testing.T) {
	err := run(context.Background(), testClient(t), []string{"exec", flagDispatch, "--force", "hello"}, io.Discard, "")
	if err == nil {
...
func TestAMessageIsStillAMessage(t *testing.T) {
	...
	if replied := mustRun(t, client, "exec", flagDispatch, "the well-known -5 degrees case"); !strings.Contains(replied, "handle ") {
...
func TestAnIdWorksWhereverANameDoes(t *testing.T) {
	...
	if replied := mustRun(t, client, "exec", address, "hello"); !strings.Contains(replied, "ok") {
