...
func TestTheManualIsOneThingToPipe(t *testing.T) {
	if err := runManual([]string{"sessions"}, &bytes.Buffer{}); err == nil {
		t.Fatal("krewe manual took an argument")
	}
}

// TestTheManualCarriesTheRealCommands, the same text `krewe` prints with no arguments, so a command
...
func TestTheManualCarriesTheRealCommands(t *testing.T) {
	...
	if !strings.Contains(out.String(), "krewe context set") {
