...
// needs.
func runKrewe(t *testing.T, client quaycrewv1.ControlPlaneServiceClient, args ...string) (string, error) {
	t.Helper()
...
func TestKreweRepositoryIsGoneAndNamesTheGitSkill(t *testing.T) {
	client := testClient(t)
	...
	for _, args := range [][]string{
		...
		for _, wants := range []string{"cloned in conversation", "git skill"} {
			if !strings.Contains(err.Error(), wants) {
				t.Errorf("krewe %s is refused with %q, want it to say %q", strings.Join(args, " "), err, wants)
			}
		}
	}
}
...
func TestTheCommandsThisReplacedSayWhereItWent(t *testing.T) {
	...
	for _, one := range []struct {
		...
		said, err := runKrewe(t, client, one.args...)
		if err == nil {
			t.Errorf("krewe %s was accepted, and said %q", strings.Join(one.args, " "), said)
			continue
		}
		if !strings.Contains(err.Error(), one.says) {
			t.Errorf("krewe %s is refused with %q, want it to say %q", strings.Join(one.args, " "), err, one.says)
		}
		if strings.Contains(err.Error(), "krewe repository") {
			t.Errorf("krewe %s is refused with %q, which points at a command that is also gone", strings.Join(one.args, " "), err)
		}
	}
