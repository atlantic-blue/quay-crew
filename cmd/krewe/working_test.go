...
// What the operator actually does while a session works: type krewe sessions, then krewe execs. Both
// screens used to say a session had been asked nothing, for as long as the job took.
//
// The exec is held open rather than timed. What is being tested is what is true while one runs, and a
// test that waits a duration for that passes on a slow machine by accident.
func TestWhatTheOperatorSeesWhileAExecRuns(t *testing.T) {
	...
	go func() {
		...
		dispatched <- run(context.Background(), client,
			[]string{"exec", "when is the electricity bill due"}, &out, "")
	}()
	select {
	case <-runner.Started:
	...
	history := mustRun(t, client, "exec", "list", identifier)
	...
	if !strings.Contains(history, "still running") {
		t.Fatalf("the history does not say the exec is still working:\n%s", history)
	}
	...
	// And it is the same exec, with its answer in it, rather than a second one underneath.
	landed := mustRun(t, client, "exec", "list", identifier)
	if strings.Contains(landed, "still running") {
		t.Fatalf("the exec still reads as working after it landed:\n%s", landed)
	}
	...
	if asked := strings.Count(landed, "when is the electricity bill due"); asked != 1 {
		t.Fatalf("the prompt appears %d times, want 1: the landing wrote a second exec\n%s", asked, landed)
	}
	if got := onlySession(t, client).GetStatus(); got != "idle" {
		t.Fatalf("the session reads %q once its exec landed, want idle", got)
	}
}
...
func TestASessionWithNoExecInItReadsIdle(t *testing.T) {
	client := testClient(t)
	mustRun(t, client, "workspace", "create", "me")
	mustRun(t, client, "project", "create", "house-bills")
	...
	if got := onlySession(t, client).GetStatus(); got != "idle" {
		t.Fatalf("a session between execs reads %q, want idle", got)
	}
