...
// session resolved it through the live listing alone, so that command was refused and the execs, the
...
func TestAnArchivedSessionsHistoryIsStillReadable(t *testing.T) {
	...
	mustRun(t, client, "exec", "read the package file")
	...
	read, err := runKrewe(t, client, "exec", "list", session.GetId()[:8])
	if err != nil {
		t.Fatalf("krewe exec list on an archived session: %v", err)
	}
	if !strings.Contains(read, "read the package file") {
		t.Fatalf("the history says %q, want the exec that was asked", read)
	}
}
...
func TestAnArchivedSessionAnswersToItsHandle(t *testing.T) {
	...
	mustRun(t, client, "workspace", "create", "me")
	mustRun(t, client, "project", "create", "house-bills")
	mustRun(t, client, "exec", "remember this")
	...
	read, err := runKrewe(t, client, "exec", "list", session.GetHandle()[:8])
	if err != nil {
		t.Fatalf("krewe exec list by handle on an archived session: %v", err)
	}
...
func TestASessionNobodyHasIsStillRefused(t *testing.T) {
	...
	_, err := runKrewe(t, client, "exec", "list", "nosuchsession")
	if err == nil {
