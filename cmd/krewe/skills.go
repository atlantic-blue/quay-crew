...
func runSkill(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: krewe skill <import|list|attach|detach>")
	}
	switch args[0] {
	case "import":
...
func runSkillImport(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe skill import <directory>")
	}
	...
	fmt.Fprintf(out, "attach it with: krewe skill attach %s\n", imported.GetName())
	return nil
}
...
func runSkillList(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if len(args) > 1 {
		return fmt.Errorf("usage: krewe skill list [<workspace, or workspace/project/session>]")
	}
	...
	read := heldBy("skills", where, "krewe skill list on its own reads what the system holds")
...
func runSkillAttach(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	// "system" where a workspace goes, the same word krewe context set takes, and it means the same
...
// one krewe context set already takes.
...
func skillAndAddress(args []string, verb string) (name, typed string, err error) {
	...
	default:
		return "", "", fmt.Errorf("usage: krewe skill %s [<workspace>] <name>", verb)
	}
}
