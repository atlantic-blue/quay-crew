...
func refused(t *testing.T, client quaycrewv1.ControlPlaneServiceClient, args ...string) error {
	...
	if err == nil {
		t.Fatalf("krewe %s was accepted: %q", strings.Join(args, " "), out.String())
	}
	return err
}
...
// The one that started this. `krewe use itv/nope` answered "workspace: no workspace with that id or
...
func TestAMissingSessionSaysWhatTheProjectHas(t *testing.T) {
	client := aSystem(t)
	mustRun(t, client, "exec", flagDispatch, "itv/fe-player", "hello")
...
func TestAnEmptyLevelSaysHowToMakeOneRatherThanListingNothing(t *testing.T) {
	...
	for _, want := range []string{"acme", "no projects yet", "krewe project create"} {
		if !strings.Contains(err.Error(), want) {
			t.Errorf("the refusal does not mention %q: %s", want, err)
		}
	}
...
func TestStandingSomewhereTheSystemNoLongerHasSaysSo(t *testing.T) {
	...
	for _, command := range [][]string{{"sessions"}, {"exec", flagDispatch, "hello"}, {"context"}} {
		err := refused(t, client, command...)
		if !strings.Contains(err.Error(), "standing in ghost/gone") {
			t.Errorf("krewe %s does not say where you are standing: %s", strings.Join(command, " "), err)
		}
		if !strings.Contains(err.Error(), "krewe use") {
			t.Errorf("krewe %s does not say how to move: %s", strings.Join(command, " "), err)
		}
	}
}
