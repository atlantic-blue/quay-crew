...
func runHeader(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer, addr string) error {
	if len(args) != 0 {
		return fmt.Errorf("usage: krewe header, and the panel runs it for you")
	}
...
// standing is read every time rather than once, because `krewe use` in the other pane moves it.
