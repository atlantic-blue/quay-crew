...
// `krewe secret set` trims, because a token gains a newline from the tool that printed it. A file's
...
// A workspace can be named the same way it is for `krewe secret set`, and standing somewhere else must
...
func TestTheUsageOffersMounting(t *testing.T) {
	...
	if err := refused(t, client, "secret"); !strings.Contains(err.Error(), "krewe secret mount") {
