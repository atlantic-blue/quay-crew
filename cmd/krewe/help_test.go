...
func TestEveryWayOfAskingForHelpIsAnswered(t *testing.T) {
	...
	for _, spelling := range []string{"help", "-h", "--help", "-help", "?"} {
		...
		if err != nil {
			t.Errorf("krewe %s was refused: %v", spelling, err)
			continue
		}
		if !strings.Contains(printed, "commands:") || !strings.Contains(printed, "exec list <session>") {
			t.Errorf("krewe %s did not print the commands: %q", spelling, printed)
		}
	}
}
...
func TestAskingForTheVersionAsAFlagNamesTheCommand(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "krewe version") {
...
func TestSessionIsACommandBecauseTheToolTeachesTheWord(t *testing.T) {
	...
	for _, spelling := range []string{"session", "sessions", "session"} {
		...
		if also != listed {
			t.Errorf("krewe %s answered differently from krewe sessions:\n%q\nwant\n%q", spelling, also, listed)
		}
	}
}
