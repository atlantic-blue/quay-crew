...
const taskUsage = "usage: krewe task [--dispatch] [<address>] <text>" +
	"\n   or: krewe task list <session>"
	...
	if len(rest) > 0 && rest[0] == "list" {
		if letGo {
			return fmt.Errorf("%s sends a task, and says nothing about reading one back"+
				"\n\n  krewe task list <session>", flagDispatch)
		}
...
// alternative is reading it out of the middle of a sentence somebody meant to send: `krewe task "say
...
func sendTask(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string,
	...
	if letGo {
		...
		fmt.Fprintf(out, "read it back with krewe task list %s, or sit in it with krewe attach %s\n",
...
//
// This is the way off `krewe tasks <session>`, which the same fingers now type as `krewe task
...
func notASessionOnItsOwn(args []string) error {
	...
	if strings.Contains(only, workspace.Separator) {
		return fmt.Errorf("%q says where, not what: a task needs something to say after the address"+
			"\n\n  krewe task %s \"...\"", only, only)
	}
	return fmt.Errorf("krewe task %s printed a session's history, and this word sends a task now"+
		"\n\n  read that session back:  krewe task list %s"+
		"\n  send it as the message:  krewe task <address> %s", only, only, only)
}
...
func runTaskList(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe task list <session>\n\na session is its id, its handle, or its address")
	}
	...
	where := narrowedTo("tasks", "session "+display.ShortID(sessionID), "krewe sessions system lists every session")
...
// task you want, and `krewe attach` is for reading it.
