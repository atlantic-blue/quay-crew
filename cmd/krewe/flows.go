...
func runFlow(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: krewe flow <import|start|schedule|unschedule|list|show|stop|answer>")
	}
	switch args[0] {
	case "import":
...
func runFlowImport(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe flow import <file>")
	}
	...
	fmt.Fprintf(out, "start a run with krewe flow start %s\n", resp.GetName())
	return nil
}
...
func runFlowStart(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if located.ProjectID == "" {
		return fmt.Errorf("a flow runs in a project: krewe flow start <workspace>/<project> %s", graph)
	}
	...
	fmt.Fprintf(out, "it dispatches tasks of its own; watch it with krewe flow show %s\n", display.ShortID(run.GetId()))
	return nil
}
...
func addressAndValue(args []string, verb, what string) (typed, value string, err error) {
	...
	default:
		return "", "", fmt.Errorf("usage: krewe flow %s [<workspace>/<project>] %s", verb, what)
	}
}
...
func runFlowList(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if len(args) > 1 {
		return fmt.Errorf("usage: krewe flow list [<workspace>/<project>]")
	}
	...
	if !readsTheSystem(typed) {
		...
		where = narrowedTo("runs", located.Path.String(), "krewe flow list system reads every project")
	}
	...
	if len(resp.GetRuns()) == 0 {
		where.nothing(out)
		fmt.Fprintf(out, "start one with krewe flow start <graph>\n")
		return nil
	}
...
func runFlowShow(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 1 {
		return fmt.Errorf("usage: krewe flow show <run>")
	}
	...
	if run.GetQuestion() != "" {
		...
		fmt.Fprintf(out, "answer it with krewe flow answer %s <answer>\n", display.ShortID(run.GetId()))
	}
	...
	// transcript.
	if run.GetJob() != "" {
		fmt.Fprintf(out, "read its steps with krewe job list --label flow.run=%s\n", run.GetId())
	}
	...
	for _, session := range flow.SessionsIn(run.GetState()) {
		fmt.Fprintf(out, "read what it did with krewe task list %s\n", display.ShortID(session))
	}
	return nil
}
...
func runFlowStop(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) < 1 || len(args) > 2 {
		return fmt.Errorf("usage: krewe flow stop <run> [<reason>]")
	}
	run, err := findFlowRun(ctx, client, args[0])
	if err != nil {
		return err
...
func runFlowAnswer(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	if len(args) != 2 {
		return fmt.Errorf("usage: krewe flow answer <run> <answer>")
	}
	run, err := findFlowRun(ctx, client, args[0])
	if err != nil {
		return err
...
func runFlowSchedule(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if located.ProjectID == "" {
		return fmt.Errorf("a flow runs in a project: krewe flow schedule <workspace>/<project> %s", graph)
	}
	...
	fmt.Fprintf(out, "the first run is one interval away; stop it with krewe flow unschedule %s\n", graph)
	return nil
}
...
func findFlowRun(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, typed string) (*quaycrewv1.FlowRun, error) {
	...
	case 0:
		return nil, fmt.Errorf("there is no run %s; krewe flow list says what has run", typed)
	default:
