...
func runTarget(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	if len(rest) > 1 {
		return fmt.Errorf("usage: quay target [<workspace>/<project>] [%s <id>] [%s <region>] [%s <arn>] [%s]",
	...
	if !located.HasProject() {
		return fmt.Errorf("a deploy target belongs to a project, and %s is a workspace: "+
			"say which body of work, for example quay target %s/<project>",
	...
	if target == nil {
		...
		fmt.Fprintf(out, "\nsay where with quay target %s %s 123456789012 %s eu-west-2 %s arn:aws:iam::123456789012:role/<name>\n",
...
// targetFlagsTaken is the flags quay target takes, for the refusal that guards every other command.
