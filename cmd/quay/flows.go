...
func runFlowShow(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	// What the run actually did is in its thread, and the summary above is the model's own account of
	// it. The two can disagree, so the way to read the turns is printed rather than left to be worked
	// out from an identifier in the state.
	if thread := run.GetState()[flow.ThreadKey]; thread != "" {
		fmt.Fprintf(out, "read what it did with quay tasks %s\n", display.ShortID(thread))
	}
	return nil
}
