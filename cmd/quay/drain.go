...
// drainAnyway is the word that drains over a task in flight. A word rather than a flag, because this
// tool has none.
...
func runDrain(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, args []string, out io.Writer) error {
	...
	case len(args) == 1 && args[0] == drainAnyway:
		force = true
	default:
		return fmt.Errorf("usage: quay drain [%s]\n\n%s drains over a task that is still working, "+
	...
	// asked for this and still has to know which conversation lost a task.
