...
func TestTheUsageNamesWeb(t *testing.T) {
	if !strings.Contains(usage, "web [<address>]") {
		t.Error("the usage does not name quay web, so nobody is told it is there")
	}
}
...
func TestWebTakesAtMostAnAddress(t *testing.T) {
	...
	if err == nil {
		t.Fatal("quay web took two addresses without complaint")
	}
	if !strings.Contains(err.Error(), "usage: quay web") {
...
func TestWebRefusesAnAddressBeyondThisMachine(t *testing.T) {
	...
	if err == nil {
		t.Fatal("quay web served an address that is reachable from another machine")
	}
