...
func TestAProjectWithNoRepositorySaysWhatToType(t *testing.T) {
	...
	for _, said := range []string{
		...
		if !strings.Contains(said, "quay project repository") {
...
func TestARepositoryThatIsNotAnOwnerAndANameIsRefusedByTheTool(t *testing.T) {
	...
	said, err := runQuay(t, client, "project", "repository", "transcript")
	if err == nil {
...
func TestAKindOfRepositoryTheSystemDoesNotKnowIsRefusedByTheTool(t *testing.T) {
	...
	said, err := runQuay(t, client, "project", "repository", "atlantic-blue/transcript", "internal")
	if err == nil {
...
func TestAKindWithNoRepositoryIsRefused(t *testing.T) {
	...
	said, err := runQuay(t, client, "project", "repository", "public")
	if err == nil {
		t.Fatalf("a kind with no repository was accepted, and said %q", said)
	}
	if !strings.Contains(err.Error(), "usage: quay project repository") {
...
func TestTheRemoteCommandNamesTheRecordAndTheGitSkill(t *testing.T) {
	...
	for _, args := range [][]string{
		...
		if !strings.Contains(err.Error(), "quay project repository") {
			t.Errorf("quay %s is refused with %q, want it to name the record", strings.Join(args, " "), err)
		}
		if !strings.Contains(err.Error(), "git skill") {
			t.Errorf("quay %s is refused with %q, want it to name the git skill", strings.Join(args, " "), err)
		}
	}
}
