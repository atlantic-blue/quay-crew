...
// firstCrewBuildThatSays is when a crew began reporting its own build. A crew older than this
...
// driftTimeout caps the extra call every command makes. The crew is on this machine or one hop away,
// so a second is generous, and a crew that is slow to answer must never hold up the command itself.
const driftTimeout = time.Second

// builds is which build each part of a crew is. Empty means that part does not say, which is not the
...
// runVersion prints which build each part of the crew is, and says where two of them differ.
//
// A crew is three parts and each is built on its own: this tool, the control plane, and the image
...
func runVersion(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, out io.Writer) error {
	...
	defer giveUp()
	// A crew that cannot be reached is not an error here. This command answers what this binary is,
	...
	if described, err := client.GetInfo(asking, &quaycrewv1.GetInfoRequest{}); err == nil {
		held.crew = described.GetVersion()
		held.sandboxImage = described.GetSandboxBuild()
	}

	fmt.Fprintf(out, "tool           %s\n", said(held.tool))
	fmt.Fprintf(out, "crew           %s\n", said(held.crew))
	...
	if held.crew == "" {
		fmt.Fprintf(out, "\nthe crew does not say which build it is: a crew built on or after %s says, "+
			"so this one is older than that. Bring it up to date with make upgrade\n", firstCrewBuildThatSays)
	}
	return nil
}
...
// build it is takes no part in this: an unknown build is not a difference, and accusing a good crew
...
func (b builds) differences() []string {
	pairs := []struct {
		...
		{"the tool and the crew", b.tool, b.crew},
		{"the tool and the sandbox image", b.tool, b.sandboxImage},
		{"the crew and the sandbox image", b.crew, b.sandboxImage},
	}
...
// reportDrift puts one line on the error stream when this tool and the crew are different builds.
//
...
//
// It never refuses a command and it never delays one for long: a crew that says nothing, or that
...
func reportDrift(ctx context.Context, client quaycrewv1.ControlPlaneServiceClient, said io.Writer) {
	...
	crew := described.GetVersion()
	if crew == "" || crew == version {
		return
	}
	fmt.Fprintf(said, "quay: the tool and the crew are different builds: %s against %s. "+
