...
// A crew keeps everything it owns in one directory. It used to be three: the data under
...
func TestEverythingACrewOwnsSitsUnderOneDirectory(t *testing.T) {
	home := t.TempDir()
	...
	if where != home {
		t.Fatalf("the crew's directory is %q, want %q", where, home)
	}

	token := crewToken(func(key string) string {
		if key == "QUAY_HOME" {
			return home
		}
		return ""
...
func TestTheDefaultDirectoryIsQuayInTheHomeDirectory(t *testing.T) {
	...
	if where != filepath.Join(home, ".quay") {
		t.Fatalf("the crew's directory defaults to %q, want %q", where, filepath.Join(home, ".quay"))
	}
	for _, retired := range []string{".quaycrew", filepath.Join(".config", "quay")} {
		if strings.Contains(where, retired) {
			t.Fatalf("the crew's directory is still %q, which holds the retired %q", where, retired)
		}
	}
}

// The way off the old layout, which is the half that gets forgotten. A crew made before the move has
// a gigabyte of transcripts and three tokens in the old place. Reading the new one would start it as
// an empty crew with a different token, and every conversation would look lost.
...
// buries the data one level deeper than anything looks for it and leaves the operator with a crew
// that still cannot find its own token. So each case asserts the whole line, both halves.
func TestACrewInTheOldLayoutIsToldExactlyWhatToMove(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatal("a crew sitting in the old layout was started as though it were new")
			}
...
// Only what is actually there is named. A crew that kept its context but never opened the panel gets
...
func TestOnlyTheFilesThatExistAreNamed(t *testing.T) {
	...
	if err == nil {
		t.Fatal("a crew with data in the old place was not stopped")
	}
	if strings.Contains(err.Error(), "panel-view") {
		t.Errorf("it names a file this crew never had:\n%s", err.Error())
	}
...
// layout. A refusal that fires on a crew with nothing to move is a refusal nobody can clear.
func TestACrewWithNothingToMoveIsNotStopped(t *testing.T) {
	for _, tc := range []struct {
		name  string
		build func(home string)
	}{
		{name: "a crew that never had the old layout", build: func(string) {}},
		{
			name: "a crew that has already moved",
			build: func(home string) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := refuseTheOldLayout(home, filepath.Join(home, ".quay")); err != nil {
				t.Fatalf("a crew with nothing to move was refused: %v", err)
			}
		})
	}
}
