...
// What one sandbox asks for, and what the crew keeps back for itself.
//
// The request is measured. One sandbox on a fourteen processor Linux runtime with 23,997 mebibytes
// was read every two seconds through its own accounting while it did the work this crew's sandboxes
...
// over the first fifty jobs a crew runs, which needs the per sandbox figure the room view already
...
// The floor under what the crew holds back for itself.
//
...
// the sandboxes hold, is the crew's own containers, and MeasuredReserve returns whichever of the two
// is larger. The floor covers the minutes after a restart, when the crew's own containers are cold
...
// one line on the machine the crew runs on, with the crew up and no jobs running:
//
//	docker stats --no-stream --format '{{.Name}}\t{{.MemUsage}}\t{{.CPUPerc}}' | grep -v quaycrew-
//
// Set QC_CREW_RESERVE_MEMORY and QC_CREW_RESERVE_PROCESSOR from what it says.
const (
	// ReserveMemory is the memory floor under the crew's own containers.
...
// DefaultReserve is the floor under what the crew keeps for itself.
