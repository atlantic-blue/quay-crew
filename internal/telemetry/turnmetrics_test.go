...
func aTurn() telemetry.TurnMeasurement {
	return telemetry.TurnMeasurement{
...
func TestATurnPublishesItsTokensAndItsCost(t *testing.T) {
	read := collected(t)
	metrics, err := telemetry.NewTurnMetrics()
	if err != nil {
		t.Fatalf("creating the instruments: %v", err)
	}

	metrics.Record(context.Background(), aTurn())
	...
	if total, found := sumFor(got, telemetry.TurnsMetric); !found || total != 1 {
		t.Errorf("turns published %v (found %v), wanted 1", total, found)
	}
}
...
func TestEveryTokenCountSaysWhoseItIsAndWhatKindItIs(t *testing.T) {
	read := collected(t)
	metrics, _ := telemetry.NewTurnMetrics()
...
// A zero that means "the backend never said" must not read as "this turn was free". The turn is
// still counted, so the turn rate stays right.
func TestATurnThatReportsNothingCountsAsATurnAndNotAsFree(t *testing.T) {
	read := collected(t)
	metrics, _ := telemetry.NewTurnMetrics()

	metrics.Record(context.Background(), telemetry.TurnMeasurement{
		Workspace: "me", Project: "house-bills", Model: "echo", Status: "idle", Reported: false,
	})

	got := read()
	if total, found := sumFor(got, telemetry.TurnsMetric); !found || total != 1 {
		t.Errorf("turns published %v (found %v), wanted the turn counted", total, found)
	}
	if _, found := sumFor(got, telemetry.TokensMetric); found {
		t.Error("a turn whose backend reported no usage published a token count, so an unknown reads as a zero")
	}
	if _, found := sumFor(got, telemetry.CostMetric); found {
		t.Error("a turn whose backend reported no usage published a cost, so an unknown reads as free")
	}
}

// A failed turn spent what it spent. Counting only the turns that worked understates the bill in
// exactly the situation somebody is investigating.
func TestAFailedTurnIsMeasuredToo(t *testing.T) {
	read := collected(t)
	metrics, _ := telemetry.NewTurnMetrics()
	...
	for _, set := range attributesOn(read(), telemetry.TokensMetric) {
		if set[telemetry.StatusAttribute] != "failed" {
			t.Errorf("a failed turn was published as %q", set[telemetry.StatusAttribute])
		}
	}
	if total, _ := sumFor(read(), telemetry.TokensMetric); total == 0 {
		t.Error("a failed turn published no tokens, so what it spent is invisible")
	}
}

// Recording on a crew with no telemetry must not be a crash. NewTurnMetrics returning nil is the
...
func TestRecordingOnNothingIsSafe(t *testing.T) {
	var absent *telemetry.TurnMetrics
	absent.Record(context.Background(), aTurn())
}
