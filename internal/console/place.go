...
// Resuming tells the console where it was standing last time. An empty place is a console that has
// nothing remembered, which opens where it opens.
...
// Same says two places name the same spot. It is what decides whether a key moved the console, and it
// compares the levels as well as the view: drilling from one workspace into another's projects leaves
// the view alone and moves the operator, and reading only the view would never write that down.
func (p Place) Same(other Place) bool {
	if p.View != other.View || p.Parent != other.Parent || len(p.Levels) != len(other.Levels) {
		return false
	}
	for at := range p.Levels {
		if p.Levels[at] != other.Levels[at] {
			return false
		}
	}
	return true
...
// it goes. It carries the level it landed on so the first screen is not blank.
