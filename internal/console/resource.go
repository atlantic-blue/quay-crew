...
type Resource struct {
	...
	// does nothing here.
	DrillTo string
	// DrillBy is the identifier DrillTo is scoped by, when it is not the row's own. Jobs is the
	// case: what a job did is its session's tasks, so enter is scoped by the session the row names
	// rather than by the job. Nil scopes by the row's own identifier, which is every other view. The
	// error is what the operator is told when there is nothing to descend to yet.
	//
	// It narrows enter alone. A key with a Descend of its own scopes by the row, because it is
	// descending somewhere else: the jobs listing opens its session's work on enter and the job's
	// own steps on S.
