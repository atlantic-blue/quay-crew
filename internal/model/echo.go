...
// exec is distinguishable from a new one in tests and smoke runs.
const echoModelSessionID = "echo-session"

// EchoRunner runs `echo` inside the session's sandbox and returns what it printed.
//
// It exists so the whole dispatch path can be exercised without a model or a subscription: the exec
...
// conversationOf is the conversation this exec ran in, as a runtime that honours the flag reports it,
// falling back to a name of its own when it was given none. The system names a conversation
// before the exec starts, so a double that answered with its own name regardless would be looser than
// the thing it stands in for: every exec would read as the runtime having ignored the flag.
