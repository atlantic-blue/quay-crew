...
const (
	// KindClaudeCode runs execs through the Claude Code command line tool inside the sandbox.
	KindClaudeCode = "claude-code"
	// KindEcho runs `echo` in the sandbox instead of a model. It is how the smoke test drives a real
	// exec end to end without a subscription.
	KindEcho = "echo"
)
...
// exec, so it holds no sandbox itself.
//
// model is which model an exec runs against, and it means nothing to the echo backend, which runs no
// model at all.
...
// The permission modes an exec can run in, which are the model's own, not ours.
const (
	...
	// It is what every exec has run as, hardcoded, since the control plane was written.
