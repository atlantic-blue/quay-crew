...
// An exec runs with the subscription token in its environment, so anything a failing exec says about
// itself is a place that token can exec up: the model's own error text, the tail of what it wrote to
// its error stream, a shell echoing the command it could not run. A tool that prints a subscription
// token into a terminal or a log because an exec failed would be a worse defect than the one it was
...
// exec payload before it is persisted.
...
// "1" would exec an explanation into nonsense.
