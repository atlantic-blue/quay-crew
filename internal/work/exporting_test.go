...
func (l *theLog) ExportWork(_ context.Context, events ...*work.Event) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
// Every movement reaches the log, and every one of them carries the work's own trace.
func TestEveryMovementIsOfferedToTheLogAndCarriesTheTrace(t *testing.T) {
	kept, plane := newRows(), newCrew()
	log := &theLog{}
	controller := work.NewController(kept, plane, nil, nil, nil).Exporting(log)
	...
	if got := kept.get(one.ID); got.Phase != work.PhaseDone {
		t.Fatalf("the work is %q", got.Phase)
	}
	...
	want := []string{work.EventClaimed, work.EventStarted, work.EventAnswered}
	...
	for _, event := range log.offered {
		if event.TraceID != aTrace {
			t.Fatalf("a %s record traces %q, and one piece of work is one trace", event.Kind, event.TraceID)
		}
	}
}
...
func TestAWriteThatDidNotApplyIsNotExported(t *testing.T) {
	...
	if got := kept.get(one.ID); got.Phase != work.PhasePending {
		t.Fatalf("the work is %q, so this test is not about a claim that failed", got.Phase)
	}
}

// A crew with no broker configured is the default, and it is the one every scenario runs on. The
// work has to run exactly the same on it.
func TestAControllerWithNoExporterStillRunsTheWork(t *testing.T) {
	...
	if got := kept.get(one.ID); got.Phase != work.PhaseDone || got.Answer == "" {
		t.Fatalf("the work is %q with answer %q on a crew with no broker", got.Phase, got.Answer)
	}
...
// The task the controller sends belongs to the work's trace. That is what lets a reader join the row
...
func TestTheTaskAControllerSendsRunsUnderTheWorksOwnTrace(t *testing.T) {
	...
	if got := telemetry.TraceIDFrom(plane.lastContext()); got != aTrace {
		t.Fatalf("the task was sent under trace %q, want the work's own", got)
	}
}

// Work nothing was tracing is dispatched all the same. A missing trace is a record with less in it,
// never a piece of work that does not run.
func TestWorkWithNoTraceStillRuns(t *testing.T) {
	kept, plane := newRows(), newCrew()
	log := &theLog{}
	controller := work.NewController(kept, plane, nil, nil, nil).Exporting(log)
	one := kept.add(declaredWork("read the electricity bill"))
	...
	for _, event := range log.offered {
		if event.TraceID != "" {
			t.Fatalf("a trace was invented for untraced work: %q", event.TraceID)
		}
	}
}
