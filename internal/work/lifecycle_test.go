...
func aLifecycleController(t *testing.T) (*work.Controller, *rows, *crew, *eyes) {
	t.Helper()
	kept, plane := newRows(), newCrew()
	plane.store = kept
	watching := &eyes{watching: map[string]bool{}}
	return work.NewController(kept, plane, nil, nil, nil).Watching(watching), kept, plane, watching
}
...
func TestASessionIdleLongerThanTheReclaimTimeGivesItsContainerBack(t *testing.T) {
	...
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 60})
...
func TestASessionInsideTheReclaimTimeKeepsItsContainer(t *testing.T) {
	controller, kept, plane, _ := aLifecycleController(t)
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 3600})
...
// Work still open holds its session alive, which is the whole reason the lifecycle is derived from
// the work rather than declared on the session.
func TestASessionWorkStillNamesIsNeverReclaimed(t *testing.T) {
	controller, kept, plane, _ := aLifecycleController(t)
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 60})
	kept.addSession(aSettledSession("session-busy", 365*24*time.Hour))
	open := declaredWork("read the electricity bill")
	open.Phase, open.Session = work.PhaseRunning, "session-busy"
	kept.add(open)

	controller.Tick(context.Background())

	if got := plane.reclaimed(); len(got) != 0 {
		t.Fatalf("the controller reclaimed %v while a piece of work still named it", got)
	}
}

// Work that ended stops holding its session, so the same session becomes a candidate.
func TestASessionOnlyEndedWorkNamesIsReclaimed(t *testing.T) {
	controller, kept, plane, _ := aLifecycleController(t)
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 60})
	kept.addSession(aSettledSession("session-finished", 365*24*time.Hour))
	done := declaredWork("read the electricity bill")
	done.Phase, done.Session = work.PhaseDone, "session-finished"
	kept.add(done)

	controller.Tick(context.Background())

	if got := plane.reclaimed(); len(got) != 1 || got[0] != "session-finished" {
		t.Fatalf("the controller reclaimed %v, want the session whose work had ended", got)
	}
}
...
func TestASessionReclaimedLongerThanTheArchiveTimeIsFiledAway(t *testing.T) {
	controller, kept, plane, _ := aLifecycleController(t)
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 60, ArchiveSeconds: 120})
...
func TestAReclaimedSessionInsideTheArchiveTimeIsLeftAlone(t *testing.T) {
	controller, kept, plane, _ := aLifecycleController(t)
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 60, ArchiveSeconds: 3600})
...
func TestASessionIsReclaimedOnOneTickAndArchivedOnALater(t *testing.T) {
	controller, kept, plane, _ := aLifecycleController(t)
	kept.allow(work.Limits{Workspace: "workspace-1", ReclaimSeconds: 60, ArchiveSeconds: 1})
