...
// A level under the mark is information and nothing else. A crew that warned about every level would
// be a crew nobody reads the warnings of.
...
func TestTheMarkIsWhereTheWarningStarts(t *testing.T) {
	for _, tc := range []struct {
		...
		if got := contextsize.Read("crew", "", body(tc.length)).Over(); got != tc.over {
			t.Errorf("%d characters reports over=%v, want %v", tc.length, got, tc.over)
		}
	}
}

// The whole finding: a hundred thousand characters at the crew level, and nothing anywhere said so.
func TestTheCrewLevelOnTheDaySaysHowBigItIsAndWhoCarriesIt(t *testing.T) {
	reading := contextsize.Read("crew", "", body(crewOnTheDay))
	if !reading.Over() {
		t.Fatalf("%d characters is not over the %d mark", crewOnTheDay, contextsize.Mark)
	}
	said := reading.Say()
	for _, want := range []string{
		"100,179",                          // the size, grouped, as a person reads it
		"20,000 character mark",            // what it is measured against
		"Every session in every workspace", // why the crew's level is the one that matters
...
func TestTheNoteIsOneLine(t *testing.T) {
	note := contextsize.Read("crew", "", body(crewOnTheDay)).Note()
	if note == "" {
		t.Fatal("the crew's level carries no note at all")
	}
	if strings.Contains(note, "\n") {
		t.Errorf("the note is more than one line:\n%s", note)
	}
	if !strings.HasPrefix(contextsize.Read("crew", "", body(crewOnTheDay)).Say(), note) {
...
func TestEachScopeNamesItsOwnReachAndItsOwnMove(t *testing.T) {
	for _, tc := range []struct {
		...
		{scope: "crew", name: "", reach: "every session in every workspace",
		...
		said := contextsize.Read(tc.scope, tc.name, body(crewOnTheDay)).Say()
...
// A level nobody has written to is the normal state of a fresh crew, and a column of noughts under a
...
func TestACountReadsTheWayAPersonSaysOne(t *testing.T) {
	for _, tc := range []struct {
		...
		{count: crewOnTheDay, want: "100,179 characters"},
