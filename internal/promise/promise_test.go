...
func TestWhatAChangeIsAskedFor(t *testing.T) {
	for _, one := range []struct {
	...
	}{
		{
			...
			files: join(edited("internal/session/waiting.go"), added("changelog.d/486-a-check.md", "features/promises.feature")),
		},
		{
		...
		},
		{
			...
			files: join(edited("internal/session/waiting.go"), added("changelog.d/486-a-check.md")),
			want:  []string{Scenario},
		},
		{
			name:  "an existing feature file gaining a scenario counts",
			files: join(edited("internal/session/waiting.go", "features/session.feature"), added("changelog.d/486-a-check.md")),
		},
		{
		...
		},
		{
			...
			files: join(edited("internal/session/waiting.go"), edited("changelog.d/README.md"), added("features/promises.feature")),
			want:  []string{ChangelogEntry},
		},
		{
			name:  "deleting the last scenario is not carrying one",
			files: join(edited("internal/session/waiting.go"), added("changelog.d/486-a-check.md"), removed("features/promises.feature")),
			want:  []string{Scenario},
		},
		{
			...
			files: join(edited("internal/session/waiting.go"), removed("changelog.d/480-fragments.md"), added("features/promises.feature")),
			want:  []string{ChangelogEntry},
		},
		{
...
func TestOnlyTheFilesThatPutItUnderTheRuleAreNamed(t *testing.T) {
	findings := Check(Change{Files: join(
		edited("internal/session/waiting.go", "docs/ARCHITECTURE.md", "internal/session/waiting_test.go"),
...
// put a session back to idle wrote thirty one lines into the shared file the week fragments landed.
func TestWritingStraightIntoTheSharedFileIsNamed(t *testing.T) {
	findings := Check(Change{Files: join(
		edited("internal/session/waiting.go", "CHANGELOG.md"),
...
func TestTheSharedFileIsOnlyNamedWhenItWasTouched(t *testing.T) {
	findings := Check(Change{Files: join(edited("internal/session/waiting.go"), added("features/promises.feature"))})
	if len(findings) != 1 {
