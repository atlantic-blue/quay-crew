...
func TestAControllerThatDiedMidTaskLeavesItsWorkToBeAdoptedOnceInPostgres(t *testing.T) {
	kept := openPostgres(t)
	...
	declared, err := dying.CreateWork(ctx, &quaycrewv1.CreateWorkRequest{
	...
	dying.TickWork(ctx)
	started, err := kept.GetWork(ctx, id)
	if err != nil {
		t.Fatalf("GetWork: %v", err)
	}
	if started.Phase != work.PhaseRunning || started.LeaseOwner != "controller-a" {
		t.Fatalf("the work is %q held by %q, want running under the controller that started it",
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("the work is %q saying %q after ten seconds", found.Phase, found.Reason)
		}
		time.Sleep(20 * time.Millisecond)
	}

	// One task, so one bill. The absence of a second start is the evidence.
	found, _ := kept.GetWork(ctx, id)
	...
	if found.Attempts != 1 {
		t.Fatalf("the work is on attempt %d, want 1", found.Attempts)
	}
	...
	if counted[work.EventStarted] != 1 {
		t.Fatalf("%d starts are on the record, want 1", counted[work.EventStarted])
	}
	if counted[work.EventReleased] != 1 || counted[work.EventClaimed] != 2 {
		t.Fatalf("the record says the work was released %d times and claimed %d, want 1 and 2",
			counted[work.EventReleased], counted[work.EventClaimed])
	}
}
...
func TestTwoControllersClaimingOneRowAtOnceLeaveOneHolderInPostgres(t *testing.T) {
	...
	declared, err := crew.CreateWork(ctx, &quaycrewv1.CreateWorkRequest{
	...
	for _, owner := range []string{"controller-a", "controller-b"} {
		go func() {
			_, err := kept.StartWork(ctx, id, work.Lease{Owner: owner, Until: time.Now().UTC().Add(time.Minute)},
				[]*work.Event{{
	...
	for range 2 {
		...
		case err == nil:
			won++
		case errors.Is(err, work.ErrNotPending):
			refused++
		default:
...
// in the life of a piece of work.
func TestTwoControllersTakingOverOneRowAtOnceLeaveOneHolderInPostgres(t *testing.T) {
	...
	for _, owner := range []string{"controller-b", "controller-c"} {
		go func() {
			_, err := kept.TakeOverWork(ctx, id, work.Lease{Owner: owner, Until: time.Now().UTC().Add(time.Minute)},
				[]*work.Event{{
					ID: store.NewID(), Kind: work.EventClaimed, Work: id,
					Workspace: workspace, Project: project, Detail: "lease_owner " + owner,
					OccurredAt: time.Now().UTC(),
	...
	for range 2 {
		...
		case err == nil:
			won++
		case errors.Is(err, work.ErrHeld):
			refused++
		default:
...
// A controller that is alive keeps its work, however many other controllers are ticking beside it.
func TestWorkUnderALeaseThatStillRunsIsNotTakenInPostgres(t *testing.T) {
	kept := openPostgres(t)
	ctx := context.Background()
	...
	declared, err := holder.CreateWork(ctx, &quaycrewv1.CreateWorkRequest{
		Project: project, Title: "read the electricity bill", Brief: "open it",
	})
	if err != nil {
		t.Fatalf("CreateWork: %v", err)
	...
	if found.LeaseOwner != "controller-a" {
		t.Fatalf("the work is held by %q, and its holder never went away", found.LeaseOwner)
	}
	tasks, _ := kept.ListTasks(ctx, found.Session, 0)
	if len(tasks) > 1 {
		t.Fatalf("%d tasks are recorded against the work, want at most the one its holder sent", len(tasks))
	}
}

// A controller that died before it dispatched left nothing behind, so the work goes back to pending
// and runs properly. Nothing was paid for, so nothing is lost.
func TestWorkAbandonedBeforeItsTaskWasSentRunsAgainInPostgres(t *testing.T) {
	kept := openPostgres(t)
	ctx := context.Background()
	crew := aCrewNamed(t, kept, "controller-a", time.Millisecond, &model.FakeRunner{Reply: "done"})
	workspace, project := aProjectOnPostgres(t, crew)
	...
	if _, err := kept.StartWork(ctx, id, work.Lease{Owner: "controller-a", Until: time.Now().UTC().Add(-time.Second)},
		[]*work.Event{{
			ID: store.NewID(), Kind: work.EventClaimed, Work: id, Workspace: workspace,
