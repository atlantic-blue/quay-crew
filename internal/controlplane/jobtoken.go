...
//
// It is the backstop and not the control. What ends a credential is the job ending: the crew takes
...
//
// Twelve hours. The longest job on this crew's record ran twenty nine minutes and the longest task
// seventeen, so nothing it has run comes near this, and a credential from a job the crew lost track
...
// forgetAfter is how long a credential that has stopped working is kept before the crew drops it.
//
// Kept rather than dropped at once, because a credential the crew cannot find is refused as a
// forgery, and that refusal sends a session looking for a fault in the token it was handed. A
// session that calls after its job ended is told the job ended, and this is how long the crew can
// still say so.
...
type minted struct {
	grant auth.Grant
	// stoppedAt is when the crew took this credential back, and zero for one it never took back. It
...
// crew took back stopped when it was taken back, and one nobody took back stopped when it ran out.
...
//
// This is what ends a credential in a working crew, and expiry is what is left for a job whose end
// the crew never saw. It is marked rather than deleted so the refusal can name the job that ended.
...
//
// It lasts as long as the job may run: the job's own deadline where it names one, and the crew's
// backstop where it does not. A session works for as long as its job does, and the crew takes the
...
func (s *Server) jobCredential(ctx context.Context, id string) (string, bool) {
	...
	// the crew's own backstop does not apply: the backstop is a guess about a job that said nothing.
	if one.Deadline != nil {
