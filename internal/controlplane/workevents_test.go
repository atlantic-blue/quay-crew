...
// refusingLog takes nothing. A broker that is down is not a reason to lose work, so this is the
...
// declaredIn is a piece of work in a fresh workspace and project.
func declaredIn(t *testing.T, server *controlplane.Server, title string) *quaycrewv1.Work {
	t.Helper()
	_, project := newProject(t, server)
	declared, err := server.CreateWork(context.Background(), &quaycrewv1.CreateWorkRequest{
		Project: project, Title: title, Brief: "open the bill and say when it is due",
	})
	if err != nil {
		t.Fatalf("CreateWork: %v", err)
	}
	return declared.GetWork()
}

// The record goes on the log after the transaction, keyed by the work so one piece of work's records
// stay in order on one partition.
func TestDeclaringWorkPutsARecordOnTheWorkStream(t *testing.T) {
	log := messaging.NewMemory()
	server, _ := crewWithLog(t, log)
	declared := declaredIn(t, server, "read the electricity bill")

	records := log.RecordsOn("acme.work")
	if len(records) != 1 {
		t.Fatalf("%d records landed on the work stream, want the declaration", len(records))
	}
	if string(records[0].Key) != declared.GetId() {
		t.Fatalf("the record is keyed by %q, and one piece of work's records have to share a partition",
			string(records[0].Key))
	}

	var event quaycrewv1.WorkEvent
	if err := proto.Unmarshal(records[0].Value, &event); err != nil {
		t.Fatalf("the record does not decode as a work event: %v", err)
	}
	if event.GetKind() != work.EventDeclared {
		t.Fatalf("the record says %q happened", event.GetKind())
	}
	if event.GetWork() != declared.GetId() || event.GetWorkspace() != declared.GetWorkspace() {
		t.Fatalf("the record names work %q in workspace %q", event.GetWork(), event.GetWorkspace())
	}
...
func TestAnExportThatFailsDoesNotFailTheWork(t *testing.T) {
	log := &refusingLog{}
	...
	found, err := kept.GetWork(context.Background(), declared.GetId())
	if err != nil {
		t.Fatalf("the work is not in the store after an export that failed: %v", err)
	}
	if found.Phase != work.PhasePending {
		t.Fatalf("the work reads %q", found.Phase)
	}
	events, err := kept.ListWorkEvents(context.Background(), declared.GetId())
	if err != nil {
	...
	if _, err := server.ListWork(context.Background(), &quaycrewv1.ListWorkRequest{}); err != nil {
...
func TestASecretInADetailReachesNeitherTheRecordNorTheLog(t *testing.T) {
	...
	events, err := kept.ListWorkEvents(context.Background(), declared.GetWork().GetId())
	if err != nil {
		t.Fatalf("ListWorkEvents: %v", err)
	}
	if len(events) != 1 {
...
// A movement of work that was stopped is a record too, and it is the one a reader comes looking for.
func TestStoppingWorkPutsItsReasonOnTheRecordAndTheLog(t *testing.T) {
	log := messaging.NewMemory()
	server, kept := crewWithLog(t, log)
	declared := declaredIn(t, server, "read the electricity bill")

	if _, err := server.StopWork(context.Background(), &quaycrewv1.StopWorkRequest{
	...
	if events[1].TraceID != events[0].TraceID {
		t.Fatalf("the two records trace %q and %q, and one piece of work is one trace",
			events[0].TraceID, events[1].TraceID)
	}
	if len(log.RecordsOn("acme.work")) != 2 {
		t.Fatalf("%d records landed on the log, want the declaration and the stop",
			len(log.RecordsOn("acme.work")))
	}
}

// One identifier joins the tree. A piece of work carries the trace of the call that declared it, so
// a reader holding the row can open the trace and every line written under it.
func TestWorkCarriesTheTraceOfTheCallThatDeclaredIt(t *testing.T) {
	server, _ := crewWithLog(t, nil)
	declared := declaredIn(t, server, "read the electricity bill")
	if len(declared.GetTraceId()) != 32 {
		t.Fatalf("the work traces %q, which joins to nothing", declared.GetTraceId())
	}
}
