...
// workTokenLife is how long a credential minted for a piece of work lasts when the work names no
// deadline. It is the length of the crew's own lease, because a credential outliving the hold on the
// work it belongs to would be a grant nobody is watching.
const workTokenLife = work.DefaultLease

// grants is every work credential this crew has minted, by token.
//
...
// workCredential mints the credential a task runs under, for the piece of work it runs.
//
// It carries the verbs the work's role declared, and it is bound to the work identifier, which
// becomes the parent of anything that session declares. A session whose work runs as no role gets a
// credential that may call nothing: default deny, so a role is what grants and nothing else is.
//
// It expires no later than the work's deadline, so a credential that leaks out of a sandbox stops
// working when the work it belongs to would have.
func (s *Server) workCredential(ctx context.Context, id string) (string, bool) {
	one, err := s.store.GetWork(ctx, id)
	if err != nil {
		return "", false
	}
	grant := auth.Grant{
		Work: one.ID, Workspace: one.Workspace, Project: one.Project,
		ExpiresAt: time.Now().UTC().Add(workTokenLife),
	}
	if one.Role != "" {
...
// Grants is what the interceptor asks to recognise a work credential.
func (s *Server) Grants() auth.Grants { return s.grants }

// credentialFor is the token a task runs under, empty for a task that runs no piece of work.
func (s *Server) credentialFor(ctx context.Context, id string) string {
	if id == "" {
		return ""
	}
	token, minted := s.workCredential(ctx, id)
	if !minted {
		return ""
	}
	return token
...
// WorkCredentialForTest mints the credential a piece of work would run under. Exported for a test,
...
func (s *Server) WorkCredentialForTest(ctx context.Context, id string) (string, bool) {
	return s.workCredential(ctx, id)
}
