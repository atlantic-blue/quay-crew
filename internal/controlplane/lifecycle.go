...
// all untouched, so the next exec builds a fresh container over the same state and the conversation
...
//
//   - running, because an exec is in flight and reclaiming would kill it with nothing saying why.
...
func (s *Server) ReclaimSession(ctx context.Context, req *quaycrewv1.ReclaimSessionRequest) (
	*quaycrewv1.ReclaimSessionResponse, error) {
	...
	case session.GetStatus() == StatusRunning:
		return nil, status.Errorf(codes.FailedPrecondition,
			"session %s has an exec under way: taking its container back now would end that exec", where)
