...
func (s *Server) taskEnv(ctx context.Context, session *quaycrewv1.Session, credential string) map[string]string {
	...
	// driver's token: an ordinary session has no business driving the crew. A session running a piece
	// of work is told the same address below, with a credential that buys it four verbs rather than
	// the crew.
	//
