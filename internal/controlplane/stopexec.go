...
// StatusExecStopped is what an exec's record says when somebody stopped it.
//
...
const StatusExecStopped = "stopped"

// stopWait is how long StopExec waits for the exec to actually end before saying it could not
// confirm the stop.
//
// The command answers only when the exec has stopped, because an operator who reads success stops
...
// the distribution of the gap between a stop being asked for and the exec record closing, over the
// first fifty stops.
const stopWait = 3 * tidyWait

// running is an exec in flight, and how to stop it.
//
// The cancel function is the whole mechanism: an exec runs the model through a context, so cancelling
...
type running struct {
	...
	// done is closed once the exec has landed, which is what lets a stop answer only when the exec
	// has really ended rather than when it was asked to.
	done chan struct{}
	mu   sync.Mutex
	// reason is what the operator said. Read under the lock because the exec goroutine reads it as it
...
// exec already stopping is not an error and does not overwrite the first reason: the first is the one
// that ended it.
...
// stopped says whether somebody asked for this exec to stop, and what they said.
...
// beginRunning registers an exec as the session's one in flight and hands back the context it runs
// under, so a stop has something to cancel.
//
// One exec per session, which is what the system already enforces everywhere else: a dispatch to a
// session continues its conversation, and two execs in one conversation at once is not a thing the
// model runtime does. A second registration replaces the first rather than being refused, because
// refusing here would fail an exec over bookkeeping.
...
// endRunning forgets the session's exec and releases the context, whatever the exec came to.
func (s *Server) endRunning(sessionID string, held *running) {
	s.runningMu.Lock()
	// Only if it is still this exec's registration. A dispatch that arrived while this one was
...
// runningIn is the exec a session has in flight, if it has one.
...
// StopExec halts the exec a session is running and keeps the session.
//
// The session survives on purpose. Its conversation, its container and its history all stay, so the
// next dispatch continues the same conversation: what the operator wanted stopped is the exec, not
// the room it was working in. StopSession is the other command, and it puts the whole session down.
//
// It answers only once the exec has actually ended. Killing the dispatch client was what people
...
func (s *Server) StopExec(ctx context.Context, req *quaycrewv1.StopExecRequest) (
	*quaycrewv1.StopExecResponse, error) {
	...
	if !inflight {
		return &quaycrewv1.StopExecResponse{Stopped: false, Session: session}, nil
	}
	...
	select {
	case <-held.done:
		return &quaycrewv1.StopExecResponse{Stopped: true, Session: s.reread(ctx, session.GetId())}, nil
	case <-waiting.Done():
		...
		return nil, status.Errorf(codes.DeadlineExceeded,
			"session %s was asked to stop and its exec has not ended yet: watch it with krewe exec list %s",
			display.ShortID(session.GetHandle()), display.ShortID(session.GetHandle()))
	}
}

// landStopped closes the record of an exec somebody stopped, and hands back the error to return.
//
// The session comes back idle rather than failed, because it is: nothing is running in it, its
// conversation is intact, and the next dispatch continues it. The exec carries "stopped" and the
// operator's own reason, so a listing tells a stop from a crash at a glance.
//
// Every write here takes a detached context. The context this exec ran under is the one the stop just
// cancelled, and a record written through it would not land, which would leave the exec reading as
// still running forever.
func (s *Server) landStopped(ctx context.Context, session *quaycrewv1.Session,
	exec *quaycrewv1.ExecEvent, spent model.Response, reason string) error {
	landing := context.WithoutCancel(ctx)
	s.recordExec(landing, session.GetId(), "", StatusIdle)
	// A stopped exec still spent what it spent up to the moment it was stopped, and a bill that
	// counts only the execs that ran to the end understates itself.
	s.measureExec(landing, session, spent, StatusExecStopped)
	s.landExec(landing, session, exec, StatusExecStopped, "", stopDetail(reason))
	return status.Errorf(codes.Canceled, "the exec in session %s was stopped: %s",
