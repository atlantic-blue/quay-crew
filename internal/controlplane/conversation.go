...
// nameConversation gives a session the name of the conversation its execs run in, and keeps it.
//
// Before the exec rather than after it, which is the whole point. The system used to pass no name at
// all on a session's first exec, read the name the runtime chose out of the output stream, and record
// it once the exec had landed. So for the whole life of that first exec the system held no name, and
...
func (s *Server) nameConversation(ctx context.Context, session *quaycrewv1.Session) string {
	...
	if err := s.store.RecordExec(ctx, session.GetId(), named, session.GetStatus()); err != nil {
		// The exec still runs, under a name nothing else can see. Worth a line, and not worth refusing
	...
	// the exec and the session disagree about which conversation this is.
...
// what decides whether the next exec starts it or resumes it.
//
...
//
// It returns the reported name only for a session the system never named, which is a session whose exec
...
// keeps no state on the host still knows a second exec must resume rather than start. It is in memory
