**A job that stops for a person is answered on the job's own view, with `a`.** Enter on a job in the
console opened the tasks of the session under it, which is one level past the thing a person pointed
at. Enter opens the job now: the job on the first line, and one line for each run of the stage it
fanned out into.
