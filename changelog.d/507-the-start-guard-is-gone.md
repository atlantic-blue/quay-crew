...
data directory under `~/.quaycrew` and the system's own data directory was not there yet. Starting
in that state would have mounted an empty directory, minted a new token, and looked exactly like a
crew that had lost every conversation. The refusal was worth having while any crew was still on the
old layout.
...
The tool keeps its own refusal, in [cmd/krewe/home.go](cmd/krewe/home.go). It says what to move and
