...
# plugin rather than part of the system, so this builds each one where it stands. Static, because the
...
COPY --from=build /out/service /service
# The skills this build ships with, so a fresh system can be given them without the operator importing
# each one by hand. They are read once, at startup, into a system whose catalogue is empty.
COPY --from=build /src/skills /skills

# The hooks this build ships with, on the same terms as the skills above: in the image so a fresh system
