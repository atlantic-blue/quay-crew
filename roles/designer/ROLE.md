...
Krewe does not stop this role from writing code or contracts, and a role session cannot put a question to the operator, so the interactive part of this brief has nothing behind it here. `CLAUDE.md`, `docs/DESIGN.md` and `docs/ASSESS.md` are files a repository may not have, and this system writes none of them.
