...
Krewe does not stop this role from editing a test file, and it does not hide the test source from it, so both prohibitions hold only if the model keeps them. `CLAUDE.md`, `references/deviation-rules.md` and `references/circuit-breaker.md` are files a repository may not have, and this system writes none of them.
