...
// anything.
//
// Reading it at all is the point. A gate that matched text would refuse a commit message that says
// what it fixed, and a refusal that is wrong costs the operator an interruption. That is worse than
// no gate. Quoting is what tells those apart: the words of a quoted argument are one token, and a
...
// the line. Each one is unwrapped rather than trusted: a signal under sudo is still a signal.
...
// conditional. `for id in 1 2 3; do kill -9 $id; done` runs a command in the middle of it, and a
// reader that stopped at `do` would find no program there at all.
var keywords = map[string]bool{
	"do": true, "then": true, "else": true, "elif": true,
	"while": true, "until": true, "if": true, "!": true,
}
