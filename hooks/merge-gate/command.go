...
// anything.
//
// Reading it at all is the point. A gate that matched text would refuse `git commit -m "merge the
// two lists"`, and a refusal that is wrong costs the operator an interruption, which is worse than
// no gate. Quoting is what tells those apart: the words of a quoted argument are one token and can
// never be a command.
