...
## home-check: refuse to start a system whose data is still in the layout from before ~/.quay
#
# The stack mounts $(QUAY_HOME)/data. A system made before the move has its tokens, its sealing key and
# every conversation under ~/.quaycrew/data, so starting would mount an empty directory, mint a new
# token, and look exactly like a system that had lost everything. The tool refuses for the same reason;
# this is the same refusal on the path that does not go through the tool.
home-check:
	@if [ -d "$(HOME)/.quaycrew/data" ] && [ ! -d "$(QUAY_HOME)/data" ]; then \
		echo "refusing: this system's data is still at $(HOME)/.quaycrew/data, and the stack now mounts"; \
