...
#
# One command, because these three go together and remembering to run the third is not a job for a
# person. The tool is what you type, the hooks are what every session runs under, and the image is
# what a session is. Leave any one of them behind and the crew is running a mix of two builds, which
# looks like a bug in whichever part you happen to be reading.
#
