...
- **A brief that asks a job to wait is refused, and the refusal names the flow.** A job was briefed to
  ...
  and `do not merge the pull request` are both still declared. A refusal that fires on ordinary work
  is the rule everybody learns to word around.

  What this does not do. It does not give a job a wait, and it does not ship the graph: the three
  nodes are still written by hand each time, and a brief worded around the rule is still a brief the
  crew cannot run.
