...
A person can say it out loud. If two people pronounce it differently, one of them will not find it.

A person can spell it from hearing it. A homophone of a common word is a trap, and so is a silent
letter.

A person can search for it and land here. A word that is already a noun inside another tool sends
every search somewhere else.
...
## fabric and blueprint: checked on 31 August 2026, both refused

Issue 585 holds the evidence, with a link for every claim. The short form is here so a reader does
not have to leave the repository.

`fabric` is carried by four large products. `danielmiessler/fabric` has 43,605 stars and is a Go
...
Three names were found that clear the bar, screened across 66 candidates: `hardstand`, `dunnage` and
`wharfinger`. Each one is free on npm, on the Python Package Index, on crates.io, in Homebrew and at
its `.dev` address. Issue 585 carries the evidence.
