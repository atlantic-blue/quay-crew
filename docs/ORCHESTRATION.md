...
crew holds a credential it cannot spend. It does not scope `job.read` to the caller's own tree yet:
the verb is checked, the tree is not. And `job.answer` is granted and refused but nothing asks a
question yet.
