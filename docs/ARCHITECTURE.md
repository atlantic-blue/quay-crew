...
the wrong shape to everybody who copies it. One of the three is the crew reading its own work rather
than an example: `pull-request-review.yaml` reads one open pull request for security, then for what
it does to the product, then for what is missing, and stops for the operator before anything is
posted.
